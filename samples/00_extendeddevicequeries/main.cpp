...
#endif
#if SYCL_EXT_INTEL_DEVICE_INFO >= 1
            if (d.has(aspect::ext_intel_gpu_eu_count)) {
                printf("\tNum EUs (direct):  %u\n",
                    d.get_info<ext::intel::info::device::gpu_eu_count>());
            }
            if (d.has(aspect::ext_intel_pci_address)) {
                auto pciAddress = d.get_info<ext::intel::info::device::pci_address>();
                printf("\tPCI Address:       %s\n", pciAddress.c_str());
            }
#endif
#if SYCL_EXT_INTEL_DEVICE_INFO >= 2
            if (d.has(aspect::ext_intel_device_info_uuid)) {
                std::array<unsigned char, 16> deviceUUID;
                deviceUUID.fill(0xCC);

                deviceUUID = d.get_info<ext::intel::info::device::uuid>();
